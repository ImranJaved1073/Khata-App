...
2. Register it in the relevant stack under `src/navigation/` (`HomeStack.tsx`, `CustomersStack.tsx`, etc.) and add its route + param type to `src/navigation/types.ts`.
3. Add any new user-facing strings to **both** `src/i18n/locales/en.json` and `ur.json` (see [`i18n.md`](i18n.md)).
4. If it replaces a `<PlaceholderScreen />`, remove the placeholder usage entirely rather than leaving it as a fallback.
...
Call `setAppLanguage("ur" | "en")` from `src/i18n/index.ts`. It flips `i18next` immediately; if the RTL direction actually changed, `I18nManager.forceRTL()` only takes full effect after the app reloads — the Settings screen (Phase 6) should prompt/trigger a reload after calling this, not assume the layout flips live.
