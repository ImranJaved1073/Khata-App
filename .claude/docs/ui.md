...
- **`ActionSheet`** — a themed bottom sheet (`Modal` + slide-up card) replacing `Alert.alert` for multi-option choices: the New Entry choice sheet and both Share sheets (statement share on `CustomerKhataScreen`, bill share on `EntryDetailScreen`). Takes a title/subtitle and a list of `{ icon, iconBackgroundColor, iconColor, label, description, onPress }` options plus an optional cancel row. This is presentation only — every option still calls the exact same navigation/share function the old `Alert.alert` button did.
