...
| Cloud backup | `@react-native-google-signin/google-signin` (Phase 7, 2026-08-08, user-requested — the deliberate exception to "no network calls" below) + plain `fetch` against the Google Drive v3 REST API (no Drive SDK). `src/lib/googleAuth.ts` wraps sign-in/out — the native module owns actual credential storage via the OS's Google account manager, so this app stores no OAuth token itself, unlike the PIN hash. `src/lib/googleDrive.ts` is a pure token-in-REST-out wrapper (find-or-create a "Khata Backups" Drive folder, upload/list/download/delete files in it). `src/lib/driveBackup.ts` composes the two into `backupToDrive`/`listDriveBackups`/`downloadDriveBackup`, called from `SettingsScreen.tsx`'s Cloud Backup section (connect/disconnect, backup now, auto-backup toggle + daily/weekly interval, restore-from-a-dated-list) and from `runAutoDriveBackupIfDue()` (`backupRepository.ts`, invoked from `App.tsx` on every unlock) for the silent scheduled path. **Needs a Google Cloud OAuth client ID only the app owner can provision, and a dev-client rebuild — see `.claude/docs/google-drive-backup-setup.md` before any of this actually works**; until then `isGoogleDriveConfigured()` keeps the Settings UI showing "Not set up yet" instead of a broken Connect button. Also fixed in the same pass: the pre-existing local (file-based) backup/restore had two real bugs — the `custom_options` table was silently missing from every export (`backupRepository.ts`'s `BackupData` now includes it, `BACKUP_VERSION` bumped 1→2, old files without it still restore), and Android SAF restore only matched the exact filename `khata-backup.json`, failing "not found" the moment a share-sheet save renamed it — `backupFile.ts`'s `pickBackupFolder()` now lists every `.json` in the chosen folder and lets `SettingsScreen` validate/disambiguate candidates instead. |
...
- **Every mutation writes an audit_log row.** Create/edit/delete on a customer, entry, or line_item is not "done" until `logAudit(...)` has been called for it. See [`skills.md`](skills.md#adding-a-mutation) for the pattern. `settings` writes (theme, language, `custom_options`, and the Cloud Backup config below) are a deliberate, standing exception — app configuration, not ledger data, so none of it goes through `logAudit()`.
...
- **Offline-first, sync-ready — with one deliberate, user-requested exception.** No network calls exist anywhere else in the app; the repository layer (`src/repositories/`) is the only thing screens talk to, and it's the seam where a future sync layer would slot in, so don't let screens query the `db` client directly. Cloud backup (above) is the one place that does reach the network — Google Sign-In + the Drive REST API — and it's opt-in (nothing happens until the user taps Connect) and additive (local SQLite via the repository layer stays the single source of truth; Drive only ever holds a point-in-time JSON export of it, never live data). Don't take this as license to add sync/network calls elsewhere without the same explicit user direction.

## Folder structure

```
src/
  db/            schema.ts (6 Drizzle tables), client.ts (expo-sqlite + drizzle), drizzle/ (generated migrations — don't hand-edit)
  ...
  lib/           pure, db-free helpers: money.ts (paisa formatting), documentFormat.ts (bill/statement HTML+text), garmentColor.ts (swatchColorFor() — the shared 3-tier colour-to-swatch resolver used by the bill editor, PDF, and WhatsApp image), share.ts (print/deep-link/share), exportData.ts (CSV/xlsx builders), exportFile.ts (write+share a file), appLock.ts (PIN hash/lockout/biometric, wraps expo-secure-store + expo-local-authentication), backupFile.ts (local backup JSON share + Android SAF restore-folder scan), googleAuth.ts (Google Sign-In wrapper), googleDrive.ts (Drive v3 REST wrapper), driveBackup.ts (composes the two into backup/list/download operations)
```
